...
class FixWidget(QWidget):
    ...
    def clickedFixBtn(self):
        ...
        if len(name) != 0 and len(password) >= 6 and len(phoneNum) == 11:
            ...
            print(self.userMessage)
            if name != self.userMessage[1]:
                self.datebase.rename(self.userMessage[1], name)
            if password != self.userMessage[4]:
                self.datebase.changeAccountPassword(self.userMessage[0], password)
            if date.toString('yyyy/MM/dd') != self.userMessage[7].strftime('%Y/%m/%d') or phoneNum != self.userMessage[8]:
                self.datebase.changeCustomerMassage(name, date.toString('yyyy/MM/dd'), phoneNum)
            self.ui.close()
